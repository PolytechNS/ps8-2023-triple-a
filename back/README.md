...
All requests are managed by `back/OnevOne.js`, which checks if the request is for a file 
(in which case it's treated by `queryManagers/front.js`) or is a REST request (managed by `queryManagers/api.js`).

Since `back/OnevOne.js` receives all the requests, it should also manage socket connections 
